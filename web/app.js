...
const neighborhoodOpacitySlider = document.getElementById("neighborhoodOpacitySlider");
const neighborhoodOpacityVal = document.getElementById("neighborhoodOpacityVal");
...
if (envelopeOpacitySlider) {
  envelopeOpacitySlider.addEventListener("input", () => {
    ...
    if (envelopeOpacityVal) envelopeOpacityVal.textContent = `${transparencyPercent}%`;
...
function updateStudyModel(geojson) {
  ensureSourcesAndLayers();
  const lotGeometry = activeLotPolygon?.length
...
function _buildRuleEngineRows(snapshot) {
  ...
  return `
    ...
    <div class="summary-row"><span>Front Yard</span><strong>${primary?.frontYard == null ? "Missing rule" : _ft(primary.frontYard)}</strong></div>
    <div class="summary-row"><span>Side Yard</span><strong>${_ft(primary?.sideYard)} × ${formatNumber(primary?.sideYardSidesRequired, 0)} side(s)</strong></div>
    <div class="summary-row"><span>Total Side Yard</span><strong>${_ft(primary?.totalSideYardRequiredFt)}</strong></div>
    <div class="summary-row"><span>Rear Yard</span><strong>${primary?.rearYard == null ? "Missing rule" : _ft(primary.rearYard)}</strong></div>
    ...
    ${_buildYardClassificationTable(lot, primary)}
    ${_buildOpenSpaceCheckRows(lot, primary)}
...
function _clipGeometryToLotBounds(geometry, lotGeometry) {
  if (!geometry || !lotGeometry) return geometry;
  ...
  try {
    ...
    return clipped?.geometry ? _normalizeStudyGeometry(clipped.geometry) : null;
  } catch (_err) {
    return null;
  }
}

function _clampFeatureCollectionToLotBounds(featureCollection, lotGeometry) {
  const source = featureCollection || EMPTY_FC;
  if (!lotGeometry) return source;
...
function _renderSelectionOverlays() {
  if (!map) return;
  ...
  if (map.getSource("selected-max-envelope")) map.getSource("selected-max-envelope").setData(EMPTY_FC);
  if (map.getSource("selected-far-envelope")) map.getSource("selected-far-envelope").setData(EMPTY_FC);
  if (map.getSource("study-model")) map.getSource("study-model").setData(EMPTY_FC);
}

function _buildEnvelopesForMultiSelectedLots() {
  if (!map?.getSource("selected-max-envelope") || !map?.getSource("selected-far-envelope") || !map?.getSource("study-model")) {
    return;
  }
  ...
  for (const feature of multiSelectedLots) {
    try {
      ...
      const lotGeometry = { type: "Polygon", coordinates: [_closeRing(lotPolygon)] };
      const lotFeature = { type: "Feature", geometry: lotGeometry, properties: data };
      const lotAnalysis = analyzeLot({
        lotFeature,
        lotRing: lotPolygon,
      ...
      if (!controlsArray.length) continue;
...
function drawPlanSVG(w, h, g, mode = "comparison") {
  ...
  pl.setAttribute("x1", "0"); pl.setAttribute("y1", "8");
  pl.setAttribute("x2", "8"); pl.setAttribute("y2", "0");
  pl.setAttribute("stroke", "#9ca3af"); pl.setAttribute("stroke-width", "1");
  pl.setAttribute("opacity", "0.5");
  pat.appendChild(pl);
  ...
  const addPoly = (ring, fill, stroke, sw, dash = "", opacity = 1) => {
    const p = pts(ring);
    if (!p) return;
...
function _isoExtrusionSVG(svg, ns, iso, ring, hFt, style) {
  ...
  for (let i = 0; i < bot.length - 1; i++) {
    const face = [bot[i], bot[i + 1], top[i + 1], top[i]];
    const el = document.createElementNS(ns, "polygon");
    el.setAttribute("points", ptStr(face));
    el.setAttribute("fill", style.sideFill || style.fill);
    el.setAttribute("stroke", style.stroke);
    el.setAttribute("stroke-width", style.lineWidth || 1.2);
    el.setAttribute("stroke-linejoin", "round");
    svg.appendChild(el);
  }

  // Bottom face
  const bf = document.createElementNS(ns, "polygon");
  bf.setAttribute("points", ptStr(bot));
  bf.setAttribute("fill", style.fill);
  bf.setAttribute("stroke", style.stroke);
  bf.setAttribute("stroke-width", style.lineWidth || 1.2);
  svg.appendChild(bf);

  // Top face (drawn last = on top)
  const tf = document.createElementNS(ns, "polygon");
  tf.setAttribute("points", ptStr(top));
  tf.setAttribute("fill", style.topFill || style.fill);
  tf.setAttribute("stroke", style.stroke);
  tf.setAttribute("stroke-width", style.lineWidth || 1.2);
  svg.appendChild(tf);
}
...
function drawIsoSVG(w, h, g, mode = "comparison") {
  ...
  const hFt = (m) => (m && m.heightFt != null) ? m.heightFt : 0;

  // Ground slab
  _isoExtrusionSVG(svg, ns, iso, g.lot, 1.5, {
